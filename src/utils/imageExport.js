...
export const exportElementToPngBlob = async (elementId) => {
    ...
    try {
        await navigator.clipboard.write([
            new window.ClipboardItem({
                'image/png': exportElementToPngBlob(elementId)
            })
        ]);
        return true;
    } catch (error) {
        ...
        try {
            const blob = await exportElementToPngBlob(elementId);
            await navigator.clipboard.write([
...
export const shareElementImage = async ({
    ...
    await exportToClipboard(elementId);
    return 'clipboard';
};
