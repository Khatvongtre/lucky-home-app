    ...
    return (
        <form onSubmit={onSave} className="space-y-4 text-left p-1">
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Lãi suất (%/năm)</label><input type="number" step="0.01" name="interestRate" required defaultValue={editingSaving?.interestRate} onChange={(e) => updateSavingCalc({ rate: Number(e.target.value) })} placeholder="VD: 8.58" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-sm outline-none focus:border-amber-500 shadow-inner" /></div>
